...
extern uint32_t LCFS_BLOCK_SIZE;   // Tamaño de bloque global (configurable)
...
typedef struct {
    ...
    uint32_t num_blocks;    // Número de bloques de datos (body blocks)
    uint32_t header_crc;
    uint32_t flags;
    uint64_t parent_oid;
...
// Extents ya no se usan; en su lugar, el bloque head contiene un array de uint64_t (números de bloque)
// que son los bloques de datos (body blocks). Esa lista se almacena inmediatamente después del nombre.
